...
def fetch_crypto_live_price(binance_symbol: str):
    """Exact last-traded price from Binance's public ticker endpoint."""
    r = requests.get(f"{BINANCE_BASE}/ticker/price", params={"symbol": binance_symbol}, timeout=8)
    r.raise_for_status()
    return float(r.json()["price"])


@st.cache_data(ttl=50, show_spinner=False)
def fetch_crypto_ohlc(binance_symbol: str, interval: str, limit: int = 300) -> pd.DataFrame:
    r = requests.get(
